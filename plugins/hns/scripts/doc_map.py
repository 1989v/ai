...
ADR-0023 Doc Index Tracking 구현체. 정책(docs/doc-index.yml)을 읽고
현재 리포지토리 상태를 스캔하여 lock 파일을 생성한다.

Usage:
  python3 doc_map.py [--repo ROOT] [--init] [--check]

- --init:  최초 실행. lock 이 없으면 생성, 있으면 덮어쓰되 report 첨부.
    ...
    for root in policy.source_roots:
        ...
        for path in root_dir.rglob("*"):
            ...
            if matches_any(rel, policy.ignore_patterns):
                continue
            if path.suffix not in policy.source_extensions:
                continue
            result[rel] = {
...
def write_lock(path: Path, lock: dict) -> None:
    header = (
        "# AUTO-GENERATED by ai/plugins/hns/scripts/doc_map.py (ADR-0023)\n"
        "# 이 파일은 검증 아티팩트다. 직접 편집하지 말 것.\n"
        "# 정책 수정은 docs/doc-index.yml 에서.\n\n"
    )
    body = yaml.safe_dump(lock, sort_keys=False, allow_unicode=True)
...
def write_init_report(path: Path, lock: dict) -> None:
    ...
    lines.append("")
    lines.append("1. `docs/doc-index.yml` 의 `manual_links` / `pattern_rules` 를 채워 coverage 향상")
    lines.append("2. 문서 상단에 `<!-- source: path/to/file.kt -->` 주석으로 explicit citation 추가")
    lines.append("3. 재실행: `python3 ai/plugins/hns/scripts/doc_map.py`")
    path.write_text("\n".join(lines))
...
def main() -> int:
    ...
    policy_path = repo / "docs" / "doc-index.yml"
    ...
    if args.check:
        if not lock_path.exists():
            ...
            return 1
        previous = yaml.safe_load(lock_path.read_text())
        current = lock
        if previous != current:
