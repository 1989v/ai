...
**OKLCH 토큰 + 핵심 React 컴포넌트** 를 모든 frontend 가 import 해 사용.

## 0. 디자인 언어

대시보드/데이터 중심 UI 를 기준으로 한 공통 시각 패턴:
다크 surface · 큰 KPI 숫자 (tabular-nums) · 의미 색상 (긍정=녹색 / 부정=빨강) · 카드 radius 12 · accent 파랑(primary) + 청록(secondary) · 행 단위 list (avatar + 메인/보조 + 우측 값 + chevron).
...
├── package.json              {{design_system}}, peerDeps react ^18||^19
...
- `scripts/sync-design-system.sh` 가 `npm pack` 결과 tarball 을
  `<fe>/vendor/` 에 복사 + 각 `package.json` 의 `{{design_system}}` 을
...
# packages/design-system 빌드 → tarball 생성 → 각 FE 의 vendor/ 갱신 + npm install
```
...
// 페이지/컴포넌트
import { KpiCard, StatCard, ListRow, SegmentControl, PrimaryButton } from '{{design_system}}';
...
5. `scripts/sync-design-system.sh` 실행 → 각 FE 의 vendor 갱신
...
- **Tarball 갱신 자동화**: pre-commit 훅이 design-system 소스 변경을 감지하면 동기화 스크립트를 자동 실행하고 각 소비 FE 의 vendor/package-lock 을 자동 staged 처리하도록 구성할 수 있다.
