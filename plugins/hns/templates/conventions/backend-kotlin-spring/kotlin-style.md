...
| 대상 | 규칙 | 예 |
|---|---|---|
| Package | 소문자, underscore 금지 | `{{base_package}}.order.domain` |
...
- 회복 탄력성(CircuitBreaker·DLQ·재시도)의 큰 그림은 프로젝트의 resilience 전략을 따른다. 트랜잭션 안에서 외부 IO 를 호출하지 않는다 (`transactional-usage.md`).
...
- 레이어 네이밍 / DI 방향 / 도메인 패턴: `code-convention.md`
- 영속성 규칙: `jpa-persistence.md`
- 로깅 규칙: `logging.md`
- API 응답 포맷: `api-format.md`
- 트랜잭션 사용 규칙: `transactional-usage.md`
