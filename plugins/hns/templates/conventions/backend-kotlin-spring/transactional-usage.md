...
nested `@Transactional` 메서드가 `@Transactional(readOnly = true)` 내부에서 예외를 던지면 `UnexpectedRollbackException` 이 발생할 수 있다. 이를 예방하기 위해 규칙을 도입한다.

## References

- TransactionalService 분리 패턴: `code-convention.md` §6
