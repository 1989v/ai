...
2-b. **흉터 목록 자기대조** — `docs/checks/common.md` 와 건드리는 모듈의 `docs/checks/{module}.md` 를 읽고(부모가 경로를 준다), 이번 코드가 각 항목에 걸리는지 스스로 대조한다. 여기 줄들은 이 프로젝트에서 실제로 터진 것이고, 구현 후 리뷰가 **같은 목록을 위반 관점으로** 다시 본다. 파일이 없으면 흉터가 아직 없다는 뜻이니 넘어간다.
