---
name: validate
description: Use to validate a project against its harness rules — docs consistency (docs → code), code compliance (rules → code), and cross-layer consistency between docs, specs, tasks, and code.
argument-hint: "[--docs | --code | --crosscheck]"
---
...
/hns:validate --crosscheck # docs·specs·tasks·code 교차 일관성 (Mode 3)
```
...
### Check 5: index.yml 보완 (doctor 용 문서 색인, 있을 때만)
...
## Mode 3: Crosscheck (`--crosscheck`)

문서·스펙·태스크·코드 간 교차 일관성. 특정 스펙을 지정하면 그 스펙만.
...
- 정책/코드 검증은 패턴 매칭 기반 샘플 확인 (100% 커버리지 아님)
- 자동 트리거는 훅으로: 커밋 전 컴파일·편집 후 린트는 `/hns:setup-hooks`, 이 스킬은 수동·파이프라인 PHASE 7 에서 실행
...
- **Calls:** 없음 (검증 전용)
- **Supersedes:** hns:doc-validate, hns:verify-crosscheck (이 스킬로 통합)
