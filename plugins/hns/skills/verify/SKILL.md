---
name: verify
description: Use after implementation to run the verification suite (standards → lint → build → tests) and record the evidence in the spec's status.md — never reports success without the command output.
...
1. Load relevant standards from `docs/standards/`
...
1. Detect project linter from `docs/architecture/overview.md` or build files (eslint, checkstyle, flake8, etc.)
2. Run linter
3. Record results
