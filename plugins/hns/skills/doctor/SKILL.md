---
name: doctor
description: Use to run the layered docs/harness health check (index integrity, agent guidance, harness alignment, evidence coverage, MCP surface) and get a PASS/WARN/FAIL score — diagnosis only.
when_to_use: 문서 헬스체크, 독터, doctor, 진단
argument-hint: "[--strict] [--output-md path] [--output-json path]"
---
...
```bash
python3 "${CLAUDE_PLUGIN_ROOT}/scripts/doctor.py" . --warn-only
```
...
- `/hns:setup-hooks` — 라이프사이클 훅 + pre-commit 훅 + CI 워크플로 설치
