...
신규 기능 개발 요청 시 `/harness-scaffold:new-feature` 파이프라인을 우선 사용한다.
스펙 작성 → 리뷰 → 태스크 분해를 거친 후 구현에 들어간다.
버그 수정, 리팩토링 등 간단한 작업은 파이프라인 없이 직접 수행 가능.

하네스 Lifecycle 커맨드:
- 같은 실수 반복 시 → `/harness-scaffold:harness-evolve`로 규칙 추가
- 주기적 청소 → `/harness-scaffold:harness-gc`
- 하네스 복잡도 점검 → `/harness-scaffold:harness-diet`
...
| Command | Purpose |
|---------|---------|
| `/harness-scaffold:new-feature` | 신규 기능 파이프라인 (shape→write→review→tasks) |
...
| `/harness-scaffold:harness-gc` | 가비지 컬렉션 (dead code, doc drift 청소) |
| `/harness-scaffold:harness-evolve` | 실패 패턴 → 규칙 인코딩 |
| `/harness-scaffold:harness-diet` | 불필요한 규칙 제거 (Bitter Lesson) |
| `/harness-scaffold:harness-audit` | 외부 벤치마크 비교 |
