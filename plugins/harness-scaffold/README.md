...
| Command | Description |
|---------|-------------|
| `/harness-scaffold:init` | 하네스 스캐폴딩 — auto-scan → doc-gen → hooks → context routing |
...
|---------|-------------|-------------|
| `/harness-scaffold:new-feature` | shape→write→review→tasks 통합 | 새 기능 시작 시 (권장) |
| `/harness-scaffold:shape-spec` | 요구사항 수집 + 스펙 폴더 초기화 | 파이프라인 없이 단계별로 할 때 |
| `/harness-scaffold:write-spec` | spec.md 작성 | shape 후 스펙 문서화 |
| `/harness-scaffold:spec-review` | 5차원 스펙 리뷰 | 스펙 품질 검증 |
| `/harness-scaffold:create-tasks` | spec → task group 분해 | 스펙 승인 후 태스크 생성 |
| `/harness-scaffold:implement-tasks` | task group 실행 (Ralph Loop) | 구현 시작 |
| `/harness-scaffold:orchestrate-tasks` | 순차/병렬 오케스트레이션 | 여러 그룹 동시 실행 |
| `/harness-scaffold:interview-capture` | 구현 전 게이트 인터뷰 | open-questions 해소 |

**일반적인 흐름:**
```
/harness-scaffold:new-feature  →  /harness-scaffold:implement-tasks  →  /harness-scaffold:verify
```

또는 단계별:
```
/harness-scaffold:shape-spec → /harness-scaffold:write-spec → /harness-scaffold:spec-review → /harness-scaffold:create-tasks → /harness-scaffold:implement-tasks
```
...
| Command | Description | When to use |
|---------|-------------|-------------|
| `/harness-scaffold:verify` | 표준 → 린트 → 빌드 → 테스트 | 구현 완료 후 |
| `/harness-scaffold:verify-crosscheck` | 6레이어 교차 일관성 검증 | docs/specs/tasks/code 간 불일치 점검 |
| `/harness-scaffold:drift-check` | 구현-스펙 불일치 감지 | 구현 중 스펙 변경 의심 시 |
...
| Command | Description | When to use |
|---------|-------------|-------------|
| `/harness-scaffold:doc-gen` | CLAUDE.md + docs/ 트리 생성 | 프로젝트 문서 초기화 |
| `/harness-scaffold:doc-validate` | docs ↔ 코드 일치 검증 | 문서 정합성 점검 |
| `/harness-scaffold:doc-html` | docs/ → HTML 사이트 생성 | 문서를 브라우저로 볼 때 |

### 하네스 자가관리 (Lifecycle)

| Command | Description | When to use |
|---------|-------------|-------------|
| `/harness-scaffold:harness-gc` | 가비지 컬렉션 | dead code, doc drift, stale rules 청소 |
| `/harness-scaffold:harness-evolve` | 실패 패턴 → 규칙 인코딩 | 같은 실수 반복 방지 |
| `/harness-scaffold:harness-diet` | 불필요한 규칙 제거 | 하네스 복잡도 점검 (Bitter Lesson) |
| `/harness-scaffold:harness-audit` | 외부 벤치마크 비교 | 다른 레포/포스트와 비교 개선 |
...
`/harness-scaffold:spec-review` 실행 시 5개 리뷰어가 순차 실행:
...
`/harness-scaffold:init` 시 프로젝트 성격에 맞는 훅 수준을 선택:
...
플러그인 설치 후 `/harness-scaffold:init`으로 프로젝트 하네스를 초기화하거나, 이미 하네스가 있는 프로젝트에서 바로 `/harness-scaffold:new-feature`으로 기능 개발을 시작한다.
