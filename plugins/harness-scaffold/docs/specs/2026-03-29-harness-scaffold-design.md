...
어떤 프로젝트든 `/hnsf:init` 한 번으로 성숙한 AI 하네스 환경을 구축하는 범용 Claude Code 플러그인.
...
커맨드 실행 시 일시 오버라이드 가능: `/hnsf:verify --mode efficient`
...
├── commands/
│   ├── init.md                  # /hnsf:init
│   ├── shape-spec.md            # /hnsf:shape-spec
│   ├── write-spec.md            # /hnsf:write-spec
│   ├── create-tasks.md          # /hnsf:create-tasks
│   ├── implement-tasks.md       # /hnsf:implement-tasks
│   ├── orchestrate-tasks.md     # /hnsf:orchestrate-tasks
│   ├── drift-check.md           # /hnsf:drift-check
│   ├── interview-capture.md     # /hnsf:interview-capture
│   ├── verify.md                # /hnsf:verify
│   └── spec-review.md           # /hnsf:spec-review
├── skills/
│   ├── core/
...
```
/hnsf:shape-spec ──→ /hnsf:write-spec ──→ /hnsf:create-tasks
       │                                         │
       │         /hnsf:interview-capture          │
       │         (구현 전 게이트)                    │
       │                ↓                         ↓
       │         /hnsf:implement-tasks ←──── /hnsf:orchestrate-tasks
       │                │
       │         /hnsf:verify
       │                │
       └──────── /hnsf:drift-check (언제든 실행 가능)
```
