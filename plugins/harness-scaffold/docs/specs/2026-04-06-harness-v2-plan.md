...
## harness-diet 연계
- `/harness-scaffold:harness-diet` 실행 시 이 프레임워크로 판단
...
## Decision
수동(/harness-scaffold:harness-gc) + 이벤트(hook) + 스케줄(cron) 3모드 지원.
...
- **Standalone:** `/harness-scaffold:doc-validate`로 직접 호출 가능
- **Calls:** 없음 (검증 전용 스킬)
```
...
Copy these files from doc-scaffolding to hns:
...
## Task 6: New SDD Commands — new-feature + verify-crosscheck

**Files:**
- Create: `commands/new-feature.md`
- Create: `commands/verify-crosscheck.md`

- [ ] **Step 1: Create `commands/new-feature.md`**
...
# /harness-scaffold:verify-crosscheck
...
```bash
cd /Users/gideok-kwon/IdeaProjects/ai
git add plugins/harness-scaffold/commands/new-feature.md plugins/harness-scaffold/commands/verify-crosscheck.md
git commit -m "feat(harness-v2): add new-feature and verify-crosscheck commands"
```
...
# Replace {{PLACEHOLDER}} values during /harness-scaffold:init
schema_version: 1
...
Run `/harness-scaffold:init` in a test context to verify the plugin is recognized with all 19 commands.
