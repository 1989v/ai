...
```
ideabank/docs/{id}-{slug}.md   # PRD 문서 (status: ready → implemented)
ideabank/tempidea.md           # Completed Ideas 섹션으로 이동
{project-root}/{service-dir}/  # 구현된 서비스 (독립 디렉토리)
```
...
구현 계획에 포함할 사항:
- 서비스 디렉토리 구조 (프로젝트 루트 하위에 독립 디렉토리로 생성)
- 기술 스택 및 의존성
- 핵심 기능별 구현 순서
- MVP 범위 (1차 구현 범위)
...
1. 프로젝트 루트 디렉토리 하위에 서비스 디렉토리 생성 (예: `quant-trader/`, `handong-pipe/`)
2. PRD의 기술 스택에 맞는 프로젝트 초기화
3. 기본 구조 scaffolding

**중요**: 각 서비스는 프로젝트 루트의 독립 디렉토리로 생성된다.
기존 서비스의 nested submodule 구조와는 별개.
...
### 6. Private Repo 분리 (visibility: private인 경우)

PRD의 `visibility`가 `private`인 경우, 구현 완료 직후:

1. `/private-repo:private-repo {서비스 디렉토리}` 를 호출하여 자동 분리
2. 분리 완료 후 다음 단계(7단계) 진행
