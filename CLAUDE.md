...
| artifact-catalog | 1 + 훅 1 | 발행한 클로드 아티팩트의 **볼트별 누적 등록부**(`claude/artifact/catalog.json`) + 프로젝트별 섹션·검색 카탈로그 페이지. `Artifact list` 50건 창 밖으로 밀린 것을 지킨다. **페이지 = 볼트 하나** — 회사·개인은 설정 구조상 한 페이지에 못 섞인다. **PostToolUse(Artifact) 훅**이 발행 직후 「/artifact-catalog 를 돌려라」를 세션에 넣어 이후 발행분이 계속 들어간다. v0.2.0 (2026-09-19) |
| media-gen | 1 + 훅 1 | AI 미디어 생성 파이프라인 — `/media-gen:create <장르>` 다섯 장르(인물 사진 변신 숏폼 · 게임 캐릭터 시트 · 게임 배경 · 이미지 · 배경 사진)를 **인테이크 게이트**(`intake.py`, 필수 리소스 없으면 종료 2) → **비용 게이트**(`budget.py`, `pricing.json` 검증일 기준 크레딧·$·₩ + ₩0 수동 경로, 사용자 선택 뒤 승인 기록) → 정체성 고정 프롬프트 → 단계별 검수 → `job.md` 로. **PreToolUse(`mcp__*higgsfield*`) 훅**이 유료 도구를 승인 기록 없이는 deny, 호출마다 승인 횟수 차감. v0.1.0 (2026-09-21) |
