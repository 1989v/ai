...
**Architecture:** 단일 플러그인 내 4-Layer 모듈 구조 (Core/SDD/Review/Project-Adaptive). 각 레이어는 skills/, commands/, agents/로 구성. 프로젝트 스캔 → 대화형 셋업 → 산출물 생성의 `/hnsf:init` 진입점.
...
All templates use `{{PLACEHOLDER}}` syntax for `/hnsf:init` to replace at generation time.
...
- /hnsf:implement-tasks, /hnsf:drift-check 문맥
...
git commit -m "feat(hnsf): add /hnsf:init entry point command"
```
