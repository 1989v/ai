...
### 4.4 io-interceptor: 통합 AOP 아키텍처
...
#### 통합 AOP 구조 (Kotlin/Spring)

개별 인터셉터(Filter, AOP, Listener 등) 대신 **1개 AOP Aspect + 경량 TraceIdFilter**로 단순화.
Clean Architecture의 adapter 패턴을 활용하여 모든 IO를 하나의 Aspect로 캡처한다.

| 컴포넌트 | 역할 |
|---------|------|
| `TraceIdFilter` | X-Trace-Id → MDC 설정만 (경량, IO 데이터 캡처 없음) |
| `IOSnapshotAspect` | 모든 IO 캡처 + 적재 (통합 AOP) |
| `IOTraceLogAppender` | 애플리케이션 로그 캡처 (기본 WARN 이상) |
...
- Spring Data Repository (JPA, ES, Redis 등) → 추가 작업 없이 자동 캡처
- WebClient 래퍼, Kafka Producer 등 → `@IOTraceable(IOType.EXTERNAL_API)` 부착
- *Adapter suffix 클래스 → 컨벤션 기반 폴백으로 자동 캡처

#### 순서 보장: sequence 채번 + 완료 시점 기록

```
AOP proceed() 전: sequence 채번 (AtomicLong, 코드 순서 보존) + invokedAt 기록
AOP proceed() 후: request + response 데이터를 포함한 전체 스냅샷을 1회 저장
```

- blocking IO (JPA): `proceed()` 반환 = 쿼리 완료
- suspend IO (WebClient `awaitSingle()` 등): Spring 6.1+ AOP가 코루틴 완료까지 대기 후 반환
- `invokedAt` ~ `completedAt` 간격으로 병목 구간 식별 가능

#### 애플리케이션 로그 캡처

Logback Appender로 요청 처리 중 발생한 로그를 같은 traceId에 적재:
- 기본 수집 레벨: WARN (설정으로 변경 가능)
- IOType.LOG로 IO 스냅샷과 동일한 Redis 구조에 저장
- sequence 순서로 IO 스냅샷 사이에 로그가 인터리빙되어 전체 흐름 파악 가능
