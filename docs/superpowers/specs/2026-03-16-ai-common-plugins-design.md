...
| `code-explore` | debug-agent에서 호출 | 코드베이스에서 관련 로직 탐색 |
...
[사용자 질의] → debug-agent
    ...
    │
    ├── 4. code-explore — 코드베이스에서 관련 로직 탐색
    │   ├── 원인 발견 → 6으로
    │   └── 코드만으로 불충분 → 5로
    │
...
1. **키 스캔 (저비용)**: `SCAN io:{traceId}:*` → 해시 키 목록에서 메서드명, 코드 라인, IO 타입으로 관련 구간 필터링
...
- **표준 인터페이스** `LogStore` 정의 → Redis(Hash + TTL), ELK, 기타 NoSQL 구현체
- Redis 구현체가 기본값. TTL로 자동 만료 관리
